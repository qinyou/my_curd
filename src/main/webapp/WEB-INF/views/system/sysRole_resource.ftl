...
<script type="text/javascript">
    ...
    function buttonUpdate(){
        ...
        if(isEmpty($('#sysMenuId').val())){
            popup.msg('请先选择菜单及按钮');
            return;
        }
        $.post('${ctx!}/sysRole/updateButtons', params, function (data) {
            if (data.state === 'ok') {
                popup.msg(data.msg, function () {
                  /*  popup.close(window.name);*/
                });
...
<div class="easyui-layout" fit="true" border="false"  >
    ...
    <div data-options="region:'east',split:true" style="width:30%;padding: 10px;"  >
        <form id="btnForm" >
            ...
            <input id="sysMenuId" type="hidden" name="sysMenuId">
            <div  id="btn-list">
            </div>
        </form>
    </div>
...
<script>
    ...
    function refreshBtnList(menuId){
        $('#sysMenuId').val(menuId);
        var url = '${ctx!}/sysRole/queryButtons?roleId=${roleId!}&menuId='+menuId;
    ...
    function loadMenuTreeChecked() {
        var url = "${ctx!}/sysRole/queryMenus?roleId=${roleId!}";
        $.getJSON(url, function (data) {
            data = easyTree.treeDataBuild(data, 'id', 'pid', 'text,state,checked,pid,iconCls,btnCount');
            $("#allMenuTree").tree({
                data: data,
                checkbox: true,
                lines: true,
