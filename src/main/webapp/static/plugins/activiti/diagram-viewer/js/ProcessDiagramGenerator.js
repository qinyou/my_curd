...
var ProcessDiagramGenerator = {
	...
	showFlowInfo: function(flow){
		...
		if(flow.conditionText){
			TPL_FLOW_INFO += '<div><b>conditionText</b>: {conditionText}</div>';
			values.conditionText = flow.conditionText;
		}
	...
	showActivityInfo: function(activity){
		...
		if(activity.getProperty("type") === 'userTask'){
			// 加入用户任务处理人信息
           if( activity.getProperty("assignee")){
			   values.assignee = activity.getProperty("assignee");
			   template += '<div><b>assignee</b>: {assignee}</div>';
		   }
		   if(activity.getProperty("candidateGroups")){
			   values.candidateGroups = activity.getProperty("candidateGroups");
			   template += '<div><b>candidateGroups</b>: {candidateGroups}</div>';
		   }
           if( activity.getProperty("candidateUsers")){
			   values.candidateUsers = activity.getProperty("candidateUsers");
			   template += '<div><b>candidateUsers</b>: {candidateUsers}</div>';
		   }
