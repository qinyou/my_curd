package com.github.qinyou.common.activiti.image;
...
public class ImageService{
     ...
     * 通过流程实例ID获取历史流程实例
     */
    private HistoricProcessInstance getHistoricProcInst(String procInstId) {
        return ActivitiUtils.getHistoryService().createHistoricProcessInstanceQuery()
     ...
     */
    private List<HistoricActivityInstance> getHistoricActivityInstAsc(String procInstId) {
        return ActivitiUtils.getHistoryService()
     ...
     */
    private List<Execution> getRunningActivityInst(String procInstId) {
        return ActivitiUtils.getRuntimeService().createExecutionQuery().processInstanceId(procInstId).list();
    }

    /**
     * 通过流程实例ID获取已经完成的历史流程实例
     */
    private List<HistoricProcessInstance> getHistoricFinishedProcInst(String procInstId) {
        return ActivitiUtils.getHistoryService().createHistoricProcessInstanceQuery().processInstanceId(procInstId).finished().list();
    }
    ...
    private List<String> getHighLightedFlows(BpmnModel bpmnModel,
        ...
        for (HistoricActivityInstance historicActivityInstance : historicActivityInstanceList) {
            // 获取流程节点
            FlowNode flowNode = (FlowNode) bpmnModel.getMainProcess().getFlowElementRecursive(historicActivityInstance.getActivityId());
        ...
        for (int k = 0; k < finishedActivityInstanceList.size(); k++) {
            ...
            currentFlowNode = (FlowNode) bpmnModel.getMainProcess().getFlowElementRecursive(currentActivityInstance.getActivityId());
            // 当前节点的所有流出线
            ...
            if ("parallelGateway".equals(currentActivityInstance.getActivityType()) || "inclusiveGateway".equals(
                ...
                for (SequenceFlow outgoingFlow : outgoingFlowList) {
                    // 获取当前节点流程线对应的下级节点
                    targetFlowNode = (FlowNode) bpmnModel.getMainProcess().getFlowElementRecursive(outgoingFlow.getTargetRef());
                ...
                // 循环当前节点的所有流出线
                // 循环所有历史节点
                log.debug("【开始】-匹配当前节点-ActivityId=【{}】需要高亮显示的流出线", currentActivityId);
                log.debug("循环历史节点");
                for (int i = 0; i < historicActivityInstanceList.size(); i++) {
                    ...
                    log.debug("第【{}/{}】个历史节点-ActivityId=[{}]", i + 1, size, historicActivityInstance.getActivityId());
                    ...
                    if (i >= k && historicActivityInstance.getActivityId().equals(currentActivityId)) {
                        log.debug("第[{}]个历史节点和当前节点一致-ActivityId=[{}]", i + 1, historicActivityInstance
                                .getActivityId());
                        ifStartFind = true;
                        // 跳过当前节点继续查找下一个节点
                        continue;
                    ...
                    if (ifStartFind) {
                        log.debug("[开始]-循环当前节点-ActivityId=【{}】的所有流出线", currentActivityId);

                        ifFinded = false;
                        for (SequenceFlow sequenceFlow : outgoingFlowList) {
                            // 如果当前节点流程线对应的下级节点在其后面的历史节点中，则该条流程线进行高亮显示
                            // 【问题】
                            log.debug("当前流出线的下级节点=[{}]", sequenceFlow.getTargetRef());
                            if (historicActivityInstance.getActivityId().equals(sequenceFlow.getTargetRef())) {
                                log.debug("当前节点[{}]需高亮显示的流出线=[{}]", currentActivityId, sequenceFlow.getId());
                                highLightedFlowIdList.add(sequenceFlow.getId());
                        ...
                        log.debug("[完成]-循环当前节点-ActivityId=【{}】的所有流出线", currentActivityId);
                    }
                    if (ifFinded) {
                ...
                log.debug("【完成】-匹配当前节点-ActivityId=【{}】需要高亮显示的流出线", currentActivityId);
    ...
    public byte[] getFlowImgByProcInstId(String procInstId) throws Exception {
        ...
        try {
            ...
            for (HistoricActivityInstance historicActivityInstance : historicActivityInstanceList) {
                ...
                log.debug("已执行的节点[{}-{}-{}-{}]", historicActivityInstance.getId(), historicActivityInstance
            ...
            for (Execution execution : runningActivityInstanceList) {
                if (StringUtils.isNotEmpty(execution.getActivityId())) {
                    ...
                    log.debug("执行中的节点[{}-{}-{}]", execution.getId(), execution.getActivityId(), execution.getName());
                }
            }
            ...
            // 获取流程定义Model对象
            BpmnModel bpmnModel = ActivitiUtils.getRepositoryService().getBpmnModel(historicProcessInstance.getProcessDefinitionId());
            ...
            imageStream = ((CustomProcessDiagramGenerator) processDiagramGenerator)
                    .generateDiagramCustom(bpmnModel, "png",
                            ...
                            ActConst.DEFAULT_FONT, ActConst.DEFAULT_FONT, ActConst.DEFAULT_FONT,
                            null, 1.0);
